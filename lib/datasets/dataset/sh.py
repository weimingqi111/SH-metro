...
class SH(data.Dataset):
    num_classes = 2
    default_resolution = [512, 512]
    mean = np.array([0.4169275,  0.43296975, 0.43430731],
                    dtype=np.float32).reshape((1, 1, 3))
    std = np.array([0.25346641, 0.2632729,  0.2665412],
                   dtype=np.float32).reshape((1, 1, 3))

    def __init__(self, opt, split):
        super(SH, self).__init__()
        self.data_dir = os.path.join(opt.data_dir, 'SH')
