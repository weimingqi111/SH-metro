...
def ctdet_decode(heat_p, wh_p, heat_h, wh_h, reg_p=None, reg_h=None, cat_spec_wh=False, K=100):
    batch_p, cat_p, height_p, width_p = heat_p.size()

    # heat = torch.sigmoid(heat)
    # perform nms on heatmaps
    heat_p = _nms(heat_p)
      
    scores_p, inds_p, clses_p, ys_p, xs_p = _topk(heat_p, K=K)

    if reg_p is not None:
      reg_p = _transpose_and_gather_feat(reg_p, inds_p)
      reg_p = reg_p.view(batch_p, K, 2)
      xs_p = xs_p.view(batch_p, K, 1) + reg_p[:, :, 0:1]
      ys_p = ys_p.view(batch_p, K, 1) + reg_p[:, :, 1:2]
    else:
      xs_p = xs_p.view(batch_p, K, 1) + 0.5
      ys_p = ys_p.view(batch_p, K, 1) + 0.5
    wh_p = _transpose_and_gather_feat(wh_p, inds_p)
    if cat_spec_wh:
      wh_p = wh_p.view(batch_p, K, cat_p, 2)
      clses_ind_p = clses_p.view(batch_p, K, 1, 1).expand(batch_p, K, 1, 2).long()
      wh_p = wh_p.gather(2, clses_ind_p).view(batch_p, K, 2)
    else:
      wh_p = wh_p.view(batch_p, K, 2)
    clses_p  = clses_p.view(batch_p, K, 1).float()
    scores_p = scores_p.view(batch_p, K, 1)
    bboxes_p = torch.cat([xs_p - wh_p[..., 0:1] / 2, 
                        ys_p - wh_p[..., 1:2] / 2,
                        xs_p + wh_p[..., 0:1] / 2, 
                        ys_p + wh_p[..., 1:2] / 2], dim=2)
    detections_p = torch.cat([bboxes_p, scores_p, clses_p], dim=2)
    
    batch_h, cat_h, height_h, width_h = heat_h.size()

    # heat = torch.sigmoid(heat)
    # perform nms on heatmaps
    heat_h = _nms(heat_h)
      
    scores_h, inds_h, clses_h, ys_h, xs_h = _topk(heat_h, K=K)

    if reg_h is not None:
      reg_h = _transpose_and_gather_feat(reg_h, inds_h)
      reg_h = reg_h.view(batch_h, K, 2)
      xs_h = xs_h.view(batch_h, K, 1) + reg_h[:, :, 0:1]
      ys_h = ys_h.view(batch_h, K, 1) + reg_h[:, :, 1:2]
    else:
      xs_h = xs_h.view(batch_h, K, 1) + 0.5
      ys_h = ys_h.view(batch_h, K, 1) + 0.5
    wh_h = _transpose_and_gather_feat(wh_h, inds_h)
    if cat_spec_wh:
      wh_h = wh_h.view(batch_h, K, cat_h, 2)
      clses_ind_h = clses_h.view(batch_h, K, 1, 1).expand(batch_h, K, 1, 2).long()
      wh_h = wh_h.gather(2, clses_ind_h).view(batch_h, K, 2)
    else:
      wh_h = wh_h.view(batch_h, K, 2)
    clses_h  = clses_h.view(batch_h, K, 1).float()
    scores_h = scores_h.view(batch_h, K, 1)
    bboxes_h = torch.cat([xs_h - wh_h[..., 0:1] / 2, 
                        ys_h - wh_h[..., 1:2] / 2,
                        xs_h + wh_h[..., 0:1] / 2, 
                        ys_h + wh_h[..., 1:2] / 2], dim=2)
    detections_h = torch.cat([bboxes_h, scores_h, clses_h], dim=2)
    return detections_p, detections_h
