...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--debug', type=int, default=0,
                             help='level of visualization.'
                                  ...
                                  #rtsp://admin:ste12345@172.16.0.51/h264/ch01/sub/av_stream
    ...
    # model
    self.parser.add_argument('--arch', default='', 
                             help='model architecture. Currently tested'
                                  'resdcn_18 | resdcn_101 |'
    ...
    self.parser.add_argument('--lr_step', type=str, default='90,120,150',
    ...
    self.parser.add_argument('--nms', action= 'store_true', default=True,
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    if opt.task == 'exdet':
      ...
      opt.heads_p = {'hm_p': 1,
                   'wh_p': 2 if not opt.cat_spec_wh else 2 * opt.num_classes}
      opt.heads_h = {'hm_h': 1,
                   'wh_h': 2 if not opt.cat_spec_wh else 2 * opt.num_classes}             
      if opt.reg_offset:
        opt.heads_p.update({'reg_p': 2})
        opt.heads_h.update({'reg_h': 2})
    ...
    print('heads_p', opt.heads_p, 'heads_h', opt.heads_h)
    return opt

  def init(self, args=''):
    default_dataset_info = {
      'ctdet': {'default_resolution': [512, 512], 'num_classes': 2, 
                'mean': [0.43513492, 0.42380644, 0.40206385], 'std': [0.25214153, 0.24766184, 0.24979096],
                'dataset': ''},
